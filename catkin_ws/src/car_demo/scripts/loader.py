...
class drivingData(TensorDataset):
    ...
    def __init__(self, directory): 
        ...
        for filename in os.listdir(directory):
            ...
            filedata = np.load(os.path.join(directory,filename), allow_pickle=True)
            self.data_lengths.append(len(filedata['images']))
            steer = np.append(steer, filedata['steer'])
            #print "data_lengths: ", self.data_lengths

        self.labels = torch.from_numpy(steer).float()
    ...
    def __getitem__(self, idx):
        ...
        imgdata = np.load(os.path.join(self.directory, self.files_list[fileidx]), allow_pickle=True)
        #extract single image/steer at the index
        image = imgdata['images'][local_idx]
        steer = imgdata['steer']
        #convert this to tensor
        image_tensor = torch.from_numpy(image)
        image_tensor = torch.reshape(image_tensor, (3, 424, 240)).float()
